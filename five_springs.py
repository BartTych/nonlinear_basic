...
def calculate_system_with_a(l_multi, n_multiplier,n_exp,amplitude):
    ...
    C[4, 4] = -1  # Spring 2: 2 ↔ 3
    C[4, 5] = +1

    C = C.tocsr()

    k = np.array([40000.0, 40000.0, 40000.0, 40000.0, 40000.0])      # stiffnesses
    a = np.array([n_exp, n_exp, n_exp, n_exp, n_exp])           # power exponent: 2.0 means nonlinear
    ...
    excitation_x, excitation_v = linear_frequency_sweep(time_steps, 5, 180, dt * steps, amplitude)
